...
# ─────────────────────────────────────────────────────────────────────────────
# Model  (single class handles both RNN and BiRNN)
# ─────────────────────────────────────────────────────────────────────────────


class RNNModel(nn.Module):
    def __init__(self, embedding_layer, hidden_dim, num_classes, bidirectional=False):
        super().__init__()
...
def plot_training_curves(history, title, path):
    ...
    axes[1].plot(epochs, history["train_f1"], label="train F1", linestyle="--")
...
def run_experiment(
    ...
    # ── Model ──────────────────────────────────────────────────────
    bidir = model_name == "birnn"
    model = RNNModel(emb_layer, hidden_dim, num_classes=5, bidirectional=bidir).to(device)

    # ── Class-balanced loss ────────────────────────────────────────
    label_counts = Counter(item["label"] for item in val_loader.dataset.data)
    ...
    history = {k: [] for k in ("train_loss", "train_acc", "train_f1", "val_acc", "val_f1")}
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for ep, (tl, ta, tf, va, vf) in enumerate(
            zip(history["train_loss"], history["train_acc"], history["train_f1"], history["val_acc"], history["val_f1"]),
            start=1,
        ):
            mlflow.log_metrics({"train_loss": tl, "train_acc": ta, "train_f1": tf, "val_acc": va, "val_f1": vf}, step=ep)
...
def run_model_experiments(args, vocab, train_loader, val_loader, test_loader, device, plots_dir):
    """
    Runs all embedding strategies for args.model (rnn or birnn).
...
def mode_train(args):
    ...
    mlflow.set_tracking_uri("file:./mlruns")  # ← ADD THIS
...
def mode_predict(args):
    ...
    bidir = args.model == "birnn"
    model = RNNModel(emb, args.hidden_dim, num_classes=5, bidirectional=bidir)
...
def main():
    parser = argparse.ArgumentParser(
        description="SST-5 sentiment classifier  –  RNN / BiRNN",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )

    # ── Core flag (required) ───────────────────────────────────────
    parser.add_argument("--model", choices=["rnn", "birnn"], required=True, help="Architecture to train or use for prediction")
    ...
    parser.add_argument("--experiment_name", type=str, default="sst5_rnn_experiments")
